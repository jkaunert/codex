...
#[test]
fn render_explicit_plugin_instructions_uses_plugin_namespace_for_skill_prefix() {
    ...
    assert_eq!(
        rendered,
        "Capabilities from the `Sample Plugin` plugin:\n- Skills from this plugin are prefixed with `sample:` in the Skills list.\nUse these plugin-associated capabilities to help solve the task."
    );
}
