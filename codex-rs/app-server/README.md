...
- `experimentalFeature/enablement/set` — patch the in-memory process-wide runtime feature enablement for the currently supported feature keys (`apps`, `plugins`, `tool_search`, `tool_suggest`, `tool_call_mcp_elicitation`, `realtime_conversation`, `workspace_dependencies`). For each feature, precedence is: cloud requirements > --enable <feature_name> > config.toml > experimentalFeature/enablement/set (new) > code default.
