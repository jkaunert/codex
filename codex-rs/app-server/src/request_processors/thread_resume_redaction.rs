...
pub(super) fn redact_thread_resume_payloads(thread: &mut Thread) {
    cap_remote_resume_turns(thread);
    ...
    thread.turns.retain(|turn| !turn.items.is_empty());
}
...
fn cap_remote_resume_turns(thread: &mut Thread) {
    let excess_turns = thread.turns.len().saturating_sub(MAX_REMOTE_RESUME_TURNS);
    if excess_turns > 0 {
        thread.turns.drain(0..excess_turns);
    }
}
